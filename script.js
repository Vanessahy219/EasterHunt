...
window.onpointerdown = (e)=> {
  ...
  gsap.timeline({defaults:{duration:0.8, ease:'back.out(4)'}})
    .to('.dagger', {rotate:2, xPercent:-10, scale:0.8}, 0)
    .to('.follower', {filter:'drop-shadow(5px 7px 2px rgba(0,0,0,0.3))'}, 0)
    .add(()=>{ //check distance between veg and dagger
      const mark = document.createElement('div')
      stageFg.append(mark)
      gsap.fromTo(mark, {innerHTML:'🗯️', x:e.x+84, y:e.y-25, rotate:'random(0,360)'}, 
      {scale:4, duration:0.1, opacity:0.8, onComplete:()=>mark.remove()})    
